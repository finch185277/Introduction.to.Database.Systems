...
std::string introMessage(int aOrder) {
  ...
  oss << "C++ implementation by Amittai Aviram -- afa13@columbia.edu -- "
         "Version "
      << VERSION << std::endl;
  oss << "Following Silberschatz, Korth, Sidarshan, Database Concepts, 5th ed."
      << std::endl;
  oss << "To build a B+ tree of a different order, start again and enter the "
         "order"
      << std::endl;
  oss << "as an integer argument:  bpt <order>  " << std::endl;
  oss << "(" << MIN_ORDER << " <= order <= " << MAX_ORDER << ")." << std::endl;
  oss << "To start with input from a file of newline-delimited integers,"
      << std::endl;
  oss << "start again and enter the order followed by the filename:"
      << std::endl;
...
std::string usageMessage() {
  std::string message =
      ...
      "\ti <k> <v> -- Insert (integer) value <v> under (integer) key <k> (<k> "
      ">= 0).\n"
...
int getOrder(int argc, const char *argv[]) {
  if (argc > 1) {
    ...
    if ((iss >> order) && iss.eof() && order >= MIN_ORDER &&
        order <= MAX_ORDER) {
      return order;
    } else {
