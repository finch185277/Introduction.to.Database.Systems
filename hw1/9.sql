...
from (
  ...
  (
    (
      select SUBSTRING_INDEX(wm.version, '.', 2) as version, convert(count(wm.match_id), unsigned) as win_cnt, convert(0, unsigned) as lose_cnt
      from (
      ...
      select SUBSTRING_INDEX(lm.version, '.', 2) as version, convert(0, unsigned) as win_cnt, convert(count(lm.match_id), unsigned) as lose_cnt
      from (
